<template>
  <div class="content">
    <div v-if="bAjax" class="list">
      <div class="item">
        <img :src="'https://cdncollege.quansuwangluo.com/' + pageData.doctorAvatar" class="avatar" alt="">
        <div class="infos">
          <div class="name">{{ pageData.doctorName }}</div>
          <div class="info chapt">{{ pageData.deptName }}</div>
          <div class="info">{{ pageData.doctorTitle }}</div>
        </div>
      ...
      <div class="conhtml" v-html="pageData.doctorIntrodution"></div>
    </div>
...
<script lang="ts">
import { getDocInfo, setCollection } from '@/api/hospital'
...
export default class DocIntro extends Vue {
  ...
  private getInfos(index: number = -1) {
    getDocInfo({ id: this.$route.params.id }).then((res: any) => {
